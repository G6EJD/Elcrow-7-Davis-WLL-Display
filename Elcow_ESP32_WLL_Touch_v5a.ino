...
// PSRAM: QPI PSRAM Enabled
// Partitiion Shceme: Huge Apa
////////////////////////////////////////////////////////////////////////////////////
String version_num = "Elcrow Davis WLL v5a";
#include <WiFi.h>
#include <time.h>
...
#include <ArduinoJson.h>  // https://github.com/bblanchon/ArduinoJson
  ...
  void setup() {
    ...
    DisplayStatus(4, "Getting Data...");
    Get_Data();
    DisplayStatus(5, "Got Data...");
    delay(1000);
    clear_screen();  
    lcd.startWrite();
  ...
  void loop() {
    ...
    if (millis() > previousMillis + loopDelay || Refresh) {
      ...
      display_text(10, 149, "SolarkW Gen = " + String(solarradiation * 1500 / 135 / 1000.0, 2) + "kWh", TFT_CYAN, 2);
      ...
      DisplayWindDirection(385, 120, winddirection, windspeed, 75, TFT_YELLOW, 2);
      gauge(75, 420, Temperature, TFT_RED, "Temperature", 0.8);
      gauge(200, 420, Humidity, TFT_BLUE, "Humidity", 0.8);
      gauge(325, 420, Dewpoint, TFT_CYAN, "Dew Point", 0.8);
      gauge(450, 420, windchill, TFT_GREEN, "WindChill", 0.8);
...
#define Inner_diameter 35
    ...
    display_text(x - (String(value, 1).length() * 7) / 2, y - 8 * zoom, String(value, 1), TFT_WHITE, 1);
  }
